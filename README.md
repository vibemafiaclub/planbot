...
이미지 전달·자체 어휘 사용·UI 위치 특정 실패·AS-IS/TO-BE 미명시·검색 가능한 앵커 부재·접근 불가능한 외부 참조 8개 렌즈로 먼저
...
**`@planbot feedback <기획내용>`**: 구현 가능성/공수 판단 없이 위 8개 렌즈로만 자료 품질을 평가하는
...
단, 티켓 본문이 Figma·위키 등 **외부 도구 링크 위주로만 구성된 경우는 되묻지 않고 그 자체를 게이트
결함으로 판정**한다("접근 불가능한 외부 참조" 렌즈) — AI도 코딩 에이전트도 링크 너머를 읽을 수 없으므로,
텍스트는 티켓 본문으로 옮기고 이미지는 캡쳐 첨부 + 이미지별 설명을 달라는 처방을 준다.
