...
이벤트 구독 필요 — 아래 준비물 참조). 채널과 동작이 다른 점:

- **멘션·스레드 불필요**: 일반 메시지만 보내면 바로 턴이 시작된다. 답장도 스레드가 아닌 최상위로 온다.
- **연속 대화**: DM 채널 전체를 하나의 대화로 취급해, 매 턴 최근 DM 히스토리 30개를 컨텍스트로 넣는다.
  이전 문답에 이어서 추가 질문하면 된다. 단 **DM 안에서 스레드 답글로 달면 무시된다** — 항상 일반
  메시지로 보낼 것.
- **모드는 메시지 단위**: 스레드처럼 모드가 고정되지 않고, 매 메시지 첫 단어가 `feedback`이면 그 턴만
  피드백 모드다. 피드백 후속 평가를 받으려면 다시 `feedback`으로 시작하면 된다.
- **`team` 커맨드**: 멘션 없이 `team`이라고 보내면 번호 목록이 오고, 다음 일반 메시지로 번호를 답하면
  등록/해제된다.
...
**멀티턴**: `thread-store.ts`가 한 번이라도 멘션된 스레드를 "활성"으로 기억한다. 이후 그 스레드에
사람이 재멘션 없이 답글만 달아도(`message` 이벤트, 봇 자신의 메시지는 필터링) 다음 턴으로 이어간다.
...
- [ ] claude가 MCP 서버를 인식 못 하고 그냥 텍스트로만 응답하고 종료하는 경우의 폴백
      (현재는 무응답으로 끝남 — 재시도/타임아웃 알림 로직 추가 여지)
